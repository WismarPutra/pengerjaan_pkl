  ...
  <div class="stat-boxes">
    ...
    <div class="tab-content" id="kebutuhan" style="display: block;">
      <table id="customers" style="margin-top: 10px; margin-left: -15px">
        ...
        <tbody>
          @foreach ($kebutuhan as $recruitment)
          <tr>
    ...
    <div class="tab-content" id="berjalan" style="display: none;">
      <table id="customers" style="margin-top: 10px; margin-left: -15px">
        <thead>
          <tr>
            <th>No</th>
            <th>{!! sortLink('namaPosisi', 'Nama Posisi', $sort, $direction) !!}</th>
            <th>{!! sortLink('regionalDirektorat', 'Regional/Direktorat', $sort, $direction) !!}</th>
            <th>{!! sortLink('band_posisi', 'Band Posisi', $sort, $direction) !!}</th>
            <th>{!! sortLink('jumlah_lowongan', 'Jumlah Lowongan', $sort, $direction) !!}</th>
            <th>{!! sortLink('target_tanggal', 'Target Tanggal Perekrutan', $sort, $direction) !!}</th>
            <th>{!! sortLink('created_by', 'Created By', $sort, $direction) !!}</th>
            <th>Actions</th>
          </tr>
        </thead>
        <tbody>
          @forelse ($berjalan as $recruitment)
          <tr>
            <td>{{ $loop->iteration }}</td>
            <td>{{ $recruitment->namaPosisi }}</td>
            <td>{{ $recruitment->regionalDirektorat }}</td>
            ...
            <td>
              ...
              <div class="dropdown-action">
                ...
                <div class="dropdown-action-content" id="dropdownActions">
                  <a href="#" class="dropdown-action-detail">Detail</a><br>
                  <a href="#" class="dropdown-action-hapus">Hapus</a><br>
                </div>
              </div>
            </td>
          </tr>
          @empty
          <tr>
            <td colspan="8" class="text-center">Belum ada data rekrutmen berjalan</td>
          </tr>
          @endforelse
        </tbody>
      </table>
    ...
    <div class="d-flex align-items-center">
      ...
      <span>entri dari {{ $kebutuhan->total() }} entri</span>
    </div>
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    // Biar fungsi showTab bisa dipakai di onclick button tab
    window.showTab = showTab;

    // Pastikan tombol tambah ada
    ...
    if (btnTambah) {
      btnTambah.addEventListener('click', function() {
        if (activeTab === 'kebutuhan') {
          ...
          const modalEl = document.getElementById('modalRekrutmen');
          if (modalEl) {
